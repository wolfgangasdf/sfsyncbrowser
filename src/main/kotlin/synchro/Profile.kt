    ...
    fun taskCompFiles(isSingleFileSync: Boolean = false) = MyTask<Boolean> {
        ...
        val taskListLocal = MyTask<Unit> {
            ...
            subfolder.subfolders.forEach {
                local!!.list(it, sync.excludeFilter.valueSafe, true, false) { vf -> acLocRem(vf, true) { vf2 -> updateMsg("found ${vf2.path}") } }
            }
        }
        val taskListRemote = MyTask<Unit> {
            updateTit("Find remote file")
            subfolder.subfolders.forEach {
                remote!!.list(it, sync.excludeFilter.valueSafe, true, false) { vf -> acLocRem(vf, false) { vf2 -> updateMsg("found ${vf2.path}") } }
            }
        }
